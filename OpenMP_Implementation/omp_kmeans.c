...
void threadingCalCluster(int tId, int N, int K, int num_threads, float *data_points, float **cluster_points, int *num_iterations){
	printf("threadingCalCluster - Thread: %d\n", tId);
...
#pragma omp barrier
		{
			printf("Thread: %d\n", tId);
			printf("current_delta: %f\n", current_delta);
			printf("current_itr_delta_global: %f\n\n", current_itr_delta_global);
...
#pragma omp master
		{
			printf("detal_global: %f\n", detal_global);
			printf("current_itr_delta_global: %f\n", current_itr_delta_global);
			printf("counter: %d\n", counter);
			detal_global = current_itr_delta_global;
	...
	printf("After updating points - Thread: %d\n", tId);
}
...
void kmeansClusterOpenMP(int N, int K, int num_threads, float *data_points, float **cluster_points, float **centroids, int *num_iterations){
	*cluster_points = (float *)malloc(sizeof(float) * N * 4);
	...
	for(int i = 0; i < K; i++){
		...
		printf("i * 3: %f\n", centroids_global[(i * 3)]);
		printf("i * 3 + 1: %f\n", centroids_global[(i * 3) + 1]);
		printf("i * 3 + 2: %f\n", centroids_global[(i * 3) + 2]);
	}
